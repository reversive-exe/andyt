...
fs.readdir(dirPath, function(err, files){
    if(err) throw err;
    let source = path.join(dirPath, files[0]);
    ...
    for(let line of lines){
        ...
        }else if(new RegExp(/(\w{1,}\s?){1,5}(=.{1,})?;/g).exec(line)){
            // Field
            let field = new Field();
...
`;
    ...
    fs.writeFile(path.join(__dirname, `out/${files[0].split(".")[0]}.smali`), code, ()=>{});
})